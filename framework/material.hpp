...
#include <string>
struct Material {
	Material(std::string const& name, Color ka, Color kd, Color ks, float m) : name(name), ka(ka), kd(kd), ks(ks), m(m), glossy(0), n(0) {};
	Material(std::string const& name, Color ka, Color kd, Color ks, float m, float glossy) : name(name), ka(ka), kd(kd), ks(ks), m(m), glossy(glossy), n() {};
