...
#if (defined(WIN32) || defined(_WIN32) || defined(__WIN32) && !defined(__CYGWIN__)) && multithreading
#include <ppl.h>
#endif
...
void Renderer::render(Scene const &scene, int frame) {
    ...
    std::cout << "Frame rendered: " << ((float) i + 1) / (float) frame * 100 << "% \t";
    std::cout << ", Elapsed time: " << elapsed_s << " s    \t, remaining time: " << (frame - i) * elapsed_s << "s\n";
    frame_times += elapsed_s;
    //ppm_.save(filename_ + "_" + std::to_string(i) + ".ppm");

    std::cout << "Rendertime total: " << frame_times << ", Rendertime per Frame: " << frame_times / frame;
}
...
Color Renderer::calculate_color(std::shared_ptr<Shape> shape, glm::vec3 const &cut, glm::vec3 const &normal,
                                Scene const &scene, Ray const &ray, int step) {
    ...
    if (shape->material()->o > 0 && shape->material()->glossy > 0) {
        ...
        Color phong = (ambient + diffuse) * (1 - shape->material()->glossy) + reflection * shape->material()->glossy +
                      specular;
        ...
        final_value = (ambient + diffuse) * (1 - shape->material()->glossy) + reflection * shape->material()->glossy +
                      specular;
...
Color Renderer::calculate_reflection(std::shared_ptr<Shape> shape, glm::vec3 const &cut, glm::vec3 const &normal,
                                     Scene const &scene, Ray const &ray, int step) {
...
Color
Renderer::calculate_refraction(std::shared_ptr<Shape> prev_shape, std::shared_ptr<Shape> shape, glm::vec3 const &cut,
                               glm::vec3 const &normal, Scene const &scene, Ray const &ray) {
    float eta = 1.0f;
...
void
Renderer::fresnel(float refraction_index, glm::vec3 const &normal_Hit, glm::vec3 const &incoming_ray, float &refr_mix) {
...
Color Renderer::calculate_diffuse(std::shared_ptr<Shape> shape, glm::vec3 const &cut, glm::vec3 const &normal,
                                  Scene const &scene) {
    ...
    for (std::shared_ptr<Light> light: scene.light_vec_) {
        ...
        bool cuts_shape = scene.root_composite_->intersect(Ray{cut + 0.2f * normal, vec_to_light}, distance, cut_point,
                                                           normal_new, cut_shape);
...
Color Renderer::calculate_specular(std::shared_ptr<Shape> shape, glm::vec3 const &cut, glm::vec3 const &normal,
                                   Scene const &scene) {
    Color comb_clr{0, 0, 0};
    std::vector<Color> light_colors{};
    for (std::shared_ptr<Light> light: scene.light_vec_) {
        ...
        //the cut can be inside the shape so a point outside the shape is calculated by cut + 0.1f*normal
        bool cuts_shape = scene.root_composite_->intersect(Ray{cut + 0.1f * normal, vec_to_light}, distance, cut_point,
                                                           normal_new, cut_shape);
        if (cuts_shape && cut_shape != nullptr) {
            can_see_light = false;
        }
